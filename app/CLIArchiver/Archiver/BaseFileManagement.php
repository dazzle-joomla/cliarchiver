...
abstract class BaseFileManagement extends Base
{
	 ...
	 * @return  void
	 */
	protected function _closeAllFiles(): void
	{
	...
	protected function fopen($file, $mode = 'wb')
	{
		if (!array_key_exists($file, $this->filePointers))
		{
			...
			if (substr($mode, 0, 1) == 'a')
			{
				...
				{
					$this->getOutput()
					     ->writeln("Truncating backup archive file $file to {$this->fileOffsets[$file]} bytes", Output::VERBOSITY_DEBUG)
					;
	 ...
	 * @return  boolean
	 */
	protected function fclose(&$fp): bool
	{
	 ...
	 * @param   integer  $maxWriteLength Maximum length of data to write
	 *
	 * @return  int  The number of bytes written
	 *
	 * @throws  ErrorException  When writing to the file is not possible
	 */
	protected function fwrite($fp, string $data, int $maxWriteLength = null): int
	{
		static $lastFp = null;
		...
		$len = is_null($maxWriteLength) ? (akstrlen($data)) : $maxWriteLength;
		...
		if (($ret === false) || (abs(($ret - $len)) >= 1))
		{
			...
			$this->getOutput()
			     ->writeln(__CLASS__ . "::_fwrite() ERROR!! Cannot write to archive file $filename. The file $fileExists. File size $currentSize bytes after writing $ret of $len bytes. Please check the output directory permissions and make sure you have enough disk space available. If this does not help, please set up a Part Size for Split Archives LOWER than this size and retry backing up.", Output::VERBOSITY_DEBUG);
	 ...
	 * @param $filename
	 */
	protected function removeFromOffsetsList(string $filename): void
	{
